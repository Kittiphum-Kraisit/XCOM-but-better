...
def choose_character_in_pygame(all_character_list, screen, list_of_team, scene_number):
    ...
    draw_text(screen, "Team " + str(scene_number-1) + " Select Characters", pygame.font.SysFont('sfcartoonisthand', 32),
              black, 250, 10)
    for i in range(10):
        if all_character_list[i].available:
            ...
            box = pygame.transform.scale(box, (all_character_list[i].rect.width + 20,
                                               all_character_list[i].rect.height + 20))
...
def choose_equipment(char_list, equipment_list, screen, control):
    ...
    draw_text(screen, f"Team {char_list[control].Team}: Select equipment for {char_list[control].Name}",
              pygame.font.SysFont('sfcartoonisthand', 34), black, (w/2)-185, 10)
    ...
    for i in range(8):
        if not equipment_list[i].clicked:
            ...
            screen.blit(pygame.image.load("pic/equip_select.png"), (equipment_list[i].rect.x - 20,
                                                                    equipment_list[i].rect.y - 15))
            draw_text(screen, f"Weapon: {equipment_list[i].name}", bigger_font, black, (w/2) - 300, (h/2) + 200)
            draw_text(screen, f"Effect: {equipment_list[i].description}", bigger_font, black,
                      (w / 2) - 300, (h / 2) + 230)
